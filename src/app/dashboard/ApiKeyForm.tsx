            ...
            {isTestnetMode && (
                <div className="absolute inset-0 z-50 bg-background/50 backdrop-blur-sm flex items-center justify-center p-6 text-center">
                    <div className="bg-white dark:bg-black/80 border border-black/10 dark:border-white/10 rounded-2xl p-6 shadow-2xl max-w-sm">
                        ...
                        <h3 className="text-lg font-bold text-foreground tracking-tight mb-2">Sandbox Mode Active</h3>
                        <p className="text-sm text-foreground/60">
                            Exchange connections are frozen while in sandbox mode. Disable Sandbox from the global toggle to connect live exchange keys.
                        </p>
                    </div>
                </div>
            )}
                        ...
                        {existingKeys.filter(k => k.isValid).map((key) => (
                            <div key={key.id} className="p-5 rounded-[1.2rem] bg-black/5 dark:bg-white/5 border border-black/5 dark:border-white/10 flex justify-between items-center transition-all hover:bg-black/10 dark:hover:bg-white/10">
                                <div className="flex items-center space-x-3">
                                    ...
                                    {key.isTestnet && (
                                        <span className="px-2.5 py-1 rounded-full bg-blue-500/10 text-blue-600 dark:text-blue-400 text-[10px] font-bold uppercase tracking-wider">Sandbox</span>
                                    )}
                                </div>
